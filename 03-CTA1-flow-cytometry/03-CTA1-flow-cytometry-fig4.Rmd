...
# select out 0Pi data
pi.dat.mutM4 <- data.mutM4 %>% 
  filter(treat == "0Pi" & !is.na(YPG) & genotype %in% c("wt","MutMsn4 #1","MutMsn4 #2")) %>%  
  select(time = Group, Sample, Plate, replicate = Replicate, genotype, median) %>%
  mutate(.,Genotype = gsub("_ko","",genotype)) %>% 
  mutate(.,new_median =median/1000) %>% 
  mutate(.,new_time = as.numeric(gsub("min","",time)))%>% 
...
pi.dat.mutM4_OL <- add_row(pi.dat,pi.dat.mutM4) %>% 
  filter(genotype %in% c("wt","MutMsn4 #1","MutMsn4 #2", "msn4_ko")) %>% 
  arrange(new_time, genotype, replicate)
```

```{r}
MutM4_color <- c("#00BFC4", "#F8766D", "#B79F00", "#E76BF3")
...
box plot + dot plot for trans TF kos CTA1-GFP
```{r}
pi.MutM4_OL.tp <- pi.dat.mutM4_OL %>% 
  filter(time %in% c("0min", "240min" ) & !is.na(median))
...
# select out 0Pi data
pi.dat.mutS7 <- data.mutS7 %>% 
  filter(treat == "0Pi" & !is.na(YPG) & genotype %in% c("wt","MutS7#1","MutS7#2")) %>%  
  select(time = Group, Sample, Plate, replicate = Replicate, genotype, median) %>%
  mutate(.,Genotype = gsub("_ko","",genotype)) %>% 
  mutate(.,new_median =median/1000) %>% 
  mutate(.,new_time = as.numeric(gsub("min","",time)))%>% 
...
pi.dat.mutS7_OL <- add_row(pi.dat,pi.dat.mutS7) %>% 
  filter(genotype %in% c("wt","MutS7#1","MutS7#2", "skn7_ko")) %>% 
  arrange(new_time, genotype, replicate)
```

```{r}
MutS7_color <- c("#B79F00", "#F8766D", "#619CFF", "#E76BF3")
...
```{r}
pi.MutS7_OL.tp <- pi.dat.mutS7_OL %>% 
  filter(time %in% c("0min", "240min" ) & !is.na(median))
