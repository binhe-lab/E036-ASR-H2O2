...
names(files) <- c("Rep1-0Pi-Ctrl","Rep2-0Pi-Ctrl","Rep1-H2O2-Rep3-0Pi", "Rep2-H2O2-Rep4-0Pi", "Rep3-H2O2-Ctrl")
...
write_csv(dat.ord, file = "gz_merge/20210708-0722-Skn7Yap1-KO-flow-data.csv")
```
...
names(tr_files) <- c("Rep1-0Pi-Ctrl","Rep2-0Pi-Ctrl","Rep1-H2O2-Rep3-0Pi", "Rep2-H2O2-Rep4-0Pi", "Rep3-H2O2-Ctrl")
...
write_csv(data, file = "gz_merge/20210708-0722-Skn7Yap1-treat-gt-KO-flow-data.csv.gz")
...
pi.dat %>% subset(., !is.na(median)) %>% 
  ...
  labs(y = "CTA1-GFP expression (MFI)", title = "C")

# only skn7 & yap1 ko
pi.dat %>% subset(., !is.na(median)&genotype == c("skn7_ko", "yap1_ko", "wt")) %>% 
  ggplot(aes(x = time, y = median, color = genotype)) + geom_point(size = 0.2) + 
  stat_summary(fun = "mean", geom = "point", size = 2) +
  stat_smooth(aes(x = as.numeric(time)), se = FALSE) +
  theme_bw() +
...
h2o2.dat %>% subset(., !is.na(median)) %>% 
  ...
  theme(legend.position="right", legend.box = "vertical", legend.text = element_text(size=10), 
        ...
        axis.text.x = element_text(angle=45,hjust=1), axis.text = element_text(size = 12), 
        plot.title = element_text(size = 14, hjust = 0)) + 
  labs(y = "CTA1-GFP expression (MFI)", title = "F")
...
write.csv(max.dat, file = "output/S7Y1-max-Rtowt.csv", row.names = FALSE)
