...
```{r}
asr_raw <- read.csv("../input/MOPO_Sc_Cg_ASR_scatter_plot _d100322.csv")
...
asr_raw  %>% subset(., Strain != "Kl" & MO.MM != 1 & PO.PM != 1) %>%
  ...
  xlab("MO/MM") +
  ggtitle("PO/PM vs MO/MM")
  ggsave("../output/MO_vs_PO_ScCg_HT.png", width = 4, height = 4, units = 'in',dpi = 300)
...
asr_raw  %>% subset(., Strain != "Kl" & Experimenter == "JL" & MO.MM != 1 & PO.PM != 1) %>%
  ...
  xlab("MO/MM") +
  ggtitle("PO/PM vs MO/MM")
  ggsave("../output/d100322_MO_vs_PO.png", width = 4, height = 4, units = 'in',dpi = 300)
...
asr_raw %>% mutate(.,ASR_Score = PO.PM/MO.MM, .keep = "all") %>% subset(., Experimenter == "JL", Strain != "Kl" & MO.MM != 1 & ASR_Score != 1) %>%
  ...
  xlab("MO/MM") +
  ggtitle("ASR effect vs MO/MM")
  ggsave("../output/d100322_ASR_vs_MOvsMM_rev_x_logy_JL.png", width = 4, height = 4, units = 'in',dpi = 300)
...
# Fold increase in survival relative to the primary stress
```{r}
