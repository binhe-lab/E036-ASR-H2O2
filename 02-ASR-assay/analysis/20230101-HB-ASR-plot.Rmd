...
p <- dat.Basal %>% 
  ...
  stat_summary(fun.data = "mean_cl_boot", geom = "pointrange", color = "red2", size = 1, 
...
p <- dat.S2 %>% 
  ...
  stat_summary(fun.data = "mean_cl_boot", geom = "crossbar", color = "red", width = 0.5) +
  #scale_y_log10() +
  facet_wrap(~Species, scales = "free_x", labeller = as_labeller(species.label)) +
...
p <- dat.S3 %>% 
  ...
  geom_point(aes(fill = Species), shape = 21,
             position = position_jitterdodge(
               jitter.width = 0.1, dodge.width = 0.5
              ), size = 2) + 
  stat_summary(aes(group = Species), position = position_dodge(0.5),
               fun.data = "mean_se", geom = "pointrange", color = "red") +
  #scale_y_log10() +
  #scale_shape_manual(values = c(21,22), labels = species.label) +
  scale_fill_manual(values = c("gray30", "white"), labels = species.label) +
  #facet_wrap(~Species, scales = "free_x", labeller = as_labeller(species.label)) +
...
p
ggsave("../output/20230103-ASR-score-across-noPi-length-sc-cg.png", width = 4, height = 3)
```
